...
call ~eot()~, possibly with a zero timeout, to issue a reply.

This interrupting *EOT* will arise because either end may call ~eot()~
at any time.  When intentionally initiated by the application, the
timeout given to ~eot()~ should be non-zero.  This assures any
messages still buffered in the sockets will be properly flushed on
both ends.  If an *EOT* reply is received prior to the timeout, this
method returns ~true~.  A number of *PAY* (if /sender/) or *DAT* (if
/recver/) messages may arrive before the reply-*EOT* and they shall be
simply discarded.
...
~.direction~ and ~.credits~ attributes of the /flow object/ are
