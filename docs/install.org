...
ZIO uses cppzmq *after* 1.5.0 which may mean installing from tip of its
git ~master~ branch.

#+begin_example
$ git clone https://github.com/gabime/spdlog.git
$ mkdir spdog/build && cd spdog/build
$ cmake .. \
...
For ZIO to find ~cppzmq~ be sure that you add its ~pkg-config~ directory
