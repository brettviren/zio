...
** Format
 ...
 The *format* field provides a hint for the parsing of the payload, and
 possibly its further interpretation.  That is, given the format,
...
** Payload
 ...
 does not dictate their use or format beyond the ZIO-reserved formats
 and providing the *format* field in the ZIO header.

 The header field *format* is intended to provide the hint needed to at
 least *parse* the payload if not also to *interpret* it.  The field *label*
 may be used to provide further hinting.

 For example, it is expected that payload of a message with *format* set
