...
begin-of-transmission (*BOT*) protocol which is a small extension to the
basic /flow protocol/ and is not exposed to remote clients.

The *BOT* protocol is illustrated in the following UML sequence diagram.
...
The object *client* is the remote client, the *port* represents ZIO port
with the broker SERVER socket.  The *BOT* protocol is inserted between
the reception of the *BOT-rem* message and its *BOT-loc* reply and
proceeds with this sequence:

1. *broker* stores the *BOT-rem* /routing ID/ into a ~cid~ attribute of the
   message flow object and reverses the flow ~direction~ attribute
   producing the *BOT-loc* message and calls the *factory* on this
   message.
2. *factory* dispatches the message in an application-dependent manner.
   ...
   what is required from them by the *BOT* protocol.
...
4. On receipt of any message with a ~cid~ the *broker* records the /routing
   ID/ of that message and the ~cid~ in that message in a bidirectional
   association. 
5. The *broker* strips the ~cid~, forwards *BOT-loc* to the remote client,
   ...
   clients.
6. Subsequent flow messages from either client can now be routed to
   its partner client.
...
The creation of the *broker* SERVER *port* is outside the scope of the *BOT*
...
the BOT protocol or else deadlock is assured.
