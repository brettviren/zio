...
{
    for (size_t ind = 0; ind < factor; ++ind) { spam += spam; }
    return spam;
}

static int compare_message(const zio::Message& m1, const zio::Message& m2)
{
    ...
    for (size_t ind = 0; ind < 4; ++ind) {
        auto& p1 = mp1[ind];
        auto& p2 = mp2[ind];
        zio::debug("compare: part {}: size1: {} size2: {}", ind, p1.size(),
                   p2.size());
...
static int do_clientish(int stype, std::string address)
{
...
int main(int argc, char* argv[])
{
    ...
    if (which == "server") { return do_serverish(ZMQ_SERVER, address); }
    if (which == "router") { return do_serverish(ZMQ_ROUTER, address); }
    if (which == "client") { return do_clientish(ZMQ_CLIENT, address); }
    if (which == "dealer") { return do_clientish(ZMQ_DEALER, address); }
    return -1;
}
