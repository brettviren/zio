...
static void client_actor(zsock_t *pipe, void *args)
{
    ...
    while (true) {
        ...
        if (!chunk) break;  //  Shutting down, quit
        chunks++;
        credit++;
...
static void server_actor(zsock_t *pipe, void *args)
{
    ...
    if (!file) {
        zsys_error(
            "This test needs a test file called 'testdata', make with:\n"
            "\ndd if=/dev/urandom of=testdata bs=1M count=1024\n");
    }
    assert(file);
    ...
    while (!zsys_interrupted) {
        ...
        if (!which or which == pipe) { break; }

        //  First frame in each message is the sender identity
        zframe_t *identity = zframe_recv(router);
        if (!identity) break;  //  Shutting down, quit
