...
auto check_recv_bot = [](auto e, FlowApp& f) {
    if (f.recv_seqno != -1) {
        zio::debug("[flow {}] check_recv_bot recv_seqno={}", f.name(),
                   f.recv_seqno);
        return false;
    }
    ...
    if (typ != "BOT") {
        zio::debug("[flow {}] check_recv_bot called with '{}'", f.name(),
                   e.msg.label());
        return false;
    }
    zio::debug("[flow {}] check_recv_bot okay with '{}'", f.name(),
               e.msg.label());
    return true;
};
...
auto check_send_bot = [](auto e, FlowApp& f) {
    if (f.send_seqno != -1) {
        zio::debug("[flow {}] check_send_bot send_seqno={}", f.name(),
                   f.send_seqno);
        return false;
    }
    ...
    std::string typ = fobj["flow"].get<std::string>();
    if (typ != "BOT") {
        zio::debug("[flow {}] check_send_bot called with '{}'", f.name(),
                   e.msg.label());
        return false;
    }
    zio::debug("[flow {}] check_send_bot okay with '{}'", f.name(),
               e.msg.label());
    return true;
};
...
auto check_pay = [](auto e, FlowApp& f) {
    ...
    std::string typ = fobj["flow"].get<std::string>();
    if (typ != "PAY") {
        zio::debug("[flow {}] check_pay not PAY '{}'", f.name(), e.msg.label());
        return false;
    }
    if (!fobj["credit"].is_number()) {
        zio::debug("[flow {}] check_pay no credit attr '{}'", f.name(),
                   e.msg.label());
        return false;
    }
    int credit = fobj["credit"];
    if (credit + f.credit > f.total_credit) {
        zio::debug("[flow {}] check_pay too much PAY {} > {}", f.name(), credit,
                   f.total_credit);
        return false;
    }
    zio::debug("[flow {}] check_dat okay with '{}'", f.name(), e.msg.label());
    return true;
};
...
auto check_dat = [](auto e, FlowApp& f) {
    ...
    std::string typ = fobj["flow"].get<std::string>();
    if (typ != "DAT") {
        zio::debug("[flow {}] check_dat not DAT '{}'", f.name(), e.msg.label());
        return false;
    }
    zio::debug("[flow {}] check_dat okay with '{}'", f.name(), e.msg.label());
    return true;
...
auto check_eot = [](auto e, FlowApp& f) {
    ...
    std::string typ = fobj["flow"].get<std::string>();
    if (typ != "EOT") {
        zio::debug("[flow {}] check_eot not EOT '{}'", f.name(), e.msg.label());
        return false;
    }
    zio::debug("[flow {}] check_eot okay with '{}'", f.name(), e.msg.label());
    return true;
};
...
auto have_credit = [](auto e, FlowApp& f) {
    zio::debug("[flow {}] have_credit {}/{}", f.name(), f.credit,
               f.total_credit);
    return f.credit > 0;
};

auto penultimate_credit = [](auto e, FlowApp& f) {
    zio::debug("[flow {}] penultimate_credit {}/{}", f.name(), f.credit,
               f.total_credit);
    return f.total_credit - f.credit == 1;
};

auto check_one_credit = [](auto e, FlowApp& f) {
    zio::debug("[flow {}] check_one_credit {}/{}", f.name(), f.credit,
               f.total_credit);
    return f.credit == 1;
};
...
auto send_msg = [](auto e, FlowApp& f) {
    ...
    f.port->send(e.msg);
    zio::debug("[flow {}] send_msg #{} with {}/{} credit, {}", f.name(),
               f.send_seqno, f.credit, f.total_credit, e.msg.label());
};
...
auto recv_bot = [](auto e, FlowApp& f) {
    ...
    zio::debug("[flow {}] recv_bot #{} as {} with {}/{} credit", f.name(),
               f.recv_seqno, dir, credit, f.total_credit);
};
...
auto flush_pay = [](auto e, FlowApp& f) {
    if (!f.credit) {
        zio::debug("[flow {}] flush_pay no credit to flush", f.port->name());
        return;
    }
    ...
    zio::debug("[flow {}] flush_pay #{}, credit:{}", f.port->name(),
               f.send_seqno, f.credit);
    f.credit = 0;
    if (f.remid.size()) { msg.set_remote_id(f.remid); }
    f.port->send(msg);
};

auto recv_msg = [](auto e, FlowApp& f) { ++f.recv_seqno; };
...
namespace {
    ...
    struct flowsm_taking
    {
        auto operator()() const noexcept
        {
            ...
            return make_transition_table(
                *state<RICH> + event<FlushPay>[have_credit] / flush_pay =
                    state<HANDSOUT>,
                state<HANDSOUT> +
                    event<RecvMsg>[penultimate_credit and check_dat] /
                        recv_msg = state<RICH>,
                state<HANDSOUT> +
                    event<RecvMsg>[!penultimate_credit and check_dat] /
                        recv_msg = state<HANDSOUT>,
                state<HANDSOUT> + event<FlushPay>[have_credit] / flush_pay =
                    state<HANDSOUT>
                // * state<TAKING> = state<WALLETCHECK>
                // , state<WALLETCHECK> + event<FlushPay> [have_credit] /
                // flush_pay = state<HANDSOUT> , state<HANDSOUT>    +
                // event<RecvMsg>  [check_dat]  / recv_msg = state<WALLETCHECK>
            );
        }
    };
    ...
    struct flowsm_giving
    {
        auto operator()() const noexcept
        {
            ...
            return make_transition_table(
                *state<BROKE> + event<RecvMsg>[check_pay] / recv_pay =
                    state<GENEROUS>,
                state<GENEROUS> +
                    event<SendMsg>[check_one_credit and check_dat] / send_msg =
                    state<BROKE>,
                state<GENEROUS> +
                    event<SendMsg>[check_many_credit and check_dat] / send_msg =
                    state<GENEROUS>,
                state<GENEROUS> + event<RecvMsg>[check_pay] / recv_pay =
                    state<GENEROUS>
                // * state<GIVING> = state<BROKE>
                // , state<BROKE>    + event<RecvMsg> [check_pay] / recv_pay =
                // state<GENEROUS> , state<GENEROUS> + event<RecvMsg>
                // [check_pay] / recv_pay = state<GENEROUS> , state<GENEROUS> +
                // event<SendMsg> [check_dat] / send_msg = state<CREDITCHECK> ,
                // state<CREDITCHECK> + event<CheckCredit> [ have_credit] =
                // state<GENEROUS> , state<CREDITCHECK> + event<CheckCredit>
                // [!have_credit] = state<BROKE>
            );
        }
    };
        ...
        auto operator()() const noexcept
        {
            ...
            return make_transition_table(
                *state<FLOWING> = state<READY>,
                state<READY> + event<BeginFlow>[is_giver] =
                    state<flowsm_giving>,
                state<READY> + event<BeginFlow>[!is_giver] =
                    state<flowsm_taking>);
        }
        ...
        auto operator()() const noexcept
        {
            ...
            return make_transition_table(
                *state<CTOR> = state<IDLE>
                ...
                ,
                state<IDLE> + event<SendMsg>[check_send_bot] / send_msg =
                    state<BOTSEND>,
                state<IDLE> + event<RecvMsg>[check_recv_bot] / recv_bot =
                    state<BOTRECV>

                ,
                state<BOTSEND> + event<RecvMsg>[check_recv_bot] / recv_bot =
                    state<flowsm_flowing>,
                state<BOTRECV> + event<SendMsg>[check_send_bot] / send_msg =
                    state<flowsm_flowing>

                ,
                state<flowsm_flowing> + event<SendMsg>[check_eot] / send_msg =
                    state<ACKFIN>,
                state<flowsm_flowing> + event<RecvMsg>[check_eot] / recv_msg =
                    state<FINACK>

                ,
                state<FINACK> + event<SendMsg>[check_eot] / send_msg =
                    state<FIN>,
                state<ACKFIN> + event<RecvMsg>[check_eot] / recv_msg =
                    state<FIN>
    ...
    struct flow_logger
    {
        ...
        void log_process_event(const TEvent&)
        {
            ...
            if (e.size() > a.size()) {
                if (e.substr(e.size() - a.size()) == a) { return; }
            }
    ...
    {
        auto src_state = std::string{
            boost::sml::aux::string<typename T::src_state>{}.c_str()};
        auto dst_state = std::string{
            boost::sml::aux::string<typename T::dst_state>{}.c_str()};
        ...
        const auto has_event =
            !boost::sml::aux::is_same<typename T::event,
                                      boost::sml::anonymous>::value;
        const auto has_guard =
            !boost::sml::aux::is_same<typename T::guard,
                                      boost::sml::front::always>::value;
        const auto has_action =
            !boost::sml::aux::is_same<typename T::action,
                                      boost::sml::front::none>::value;

        if (has_event || has_guard || has_action) { std::cout << " :"; }

        if (has_event) {
            std::cout << " "
                      << boost::sml::aux::get_type_name<typename T::event>();
        }
    ...
    typedef boost::sml::sm<flowsm, boost::sml::logger<flow_logger>,
...
struct FlowDevice
{
    ...
    void clear() { msg.set_label_object({}); }
    std::string flowtype()
    {
    ...
    void bot_handshake()
    {
        ...
        assert(sm.is<decltype(boost::sml::state<flowsm_flowing>)>(
    ...
    {
        zio::debug("[flow {}] serverish bot handshake", port->name());
        ...
        if (direction == "inject") { assert(other_dir == "extract"); }
        else {
    ...
    void bot_handshake_clientish()
    {
        msg.set_label_object(
            {{"flow", "BOT"}, {"direction", direction}, {"credit", credit}});
        zio::debug("[flow {}] clientish bot handshake label:{}", port->name(),
                   msg.label());
    ...
    {  // call if receive EOT
        ...
        auto ok = sm.process_event(SendMsg{msg});
        assert(ok);
        assert(sm.is(boost::sml::state<ACKFIN>) or
               sm.is(boost::sml::state<FIN>));
    }
    void shutdown(zio::timeout_t timeout = {})
    {  // call to initiate shutdown handshake
        zio::debug("[flow {}] shutdown", port->name());
        sendeot();
        do {
            auto what = recv(timeout);
            if (!what) {
                zio::debug("[flow {}] shutdown timeout on EOT reply",
                           port->name());
                break;
            }
    ...
    {
        if (direction == "inject") { sendpay(); }
        if (!recv(timeout)) { return nullptr; }
        std::string typ = flowtype();
        if (direction == "inject") {
            if (typ == "DAT") { return "DAT"; }
            if (typ == "PAY") { return nullptr; }
        }
        if (direction == "extract") {
            if (typ == "DAT") { return nullptr; }
            if (typ == "PAY") { return "PAY"; }
        }
        if (typ == "EOT") {
            sendeot();
            return "EOT";
    ...
    [[nodiscard]] bool recv(zio::timeout_t timeout = {})
    {
        clear();
        {
        ...
        }
        {
            ...
            if (!ok) {
                zio::debug("[flow {}] sm recv fail for: {}", port->name(),
                           msg.label());
                return false;
            }
        }
                          ...
                          int credit)
{
    ...
    zio::debug("[{} {}] direction: {}, socket: {}, sender: {}, credit: {}",
               nodename, portname, direction, zio::sock_type_name(socket),
               sender, credit);
    ...
    while (true) {
        ...
        if (sender and device.app.credit) { device.senddat(); }
...
void test_longhand()
{
    ...
    // CLIENT: initial BOT
    {
        ...
        cbot.set_label_object(
            {{"flow", "BOT"}, {"direction", "extract"}, {"credit", 2}});
    ...
    // SERVER: do a recv, better be a BOT
    {
        ...
        assert(server.sm.is<decltype(sml::state<flowsm_flowing>)>(
            sml::state<READY>));
    ...
    // CLIENT do a recv, better be a bot
    {
        ...
        assert(client.sm.is<decltype(sml::state<flowsm_flowing>)>(
            sml::state<READY>));
    ...
    // Explicitly leave READY state
    {
        ...
        assert(ok);
        assert(server.sm.is<decltype(sml::state<flowsm_taking>)>(
            sml::state<HANDSOUT>));
    }

    {
        ...
        assert(ok);
        assert(client.sm.is<decltype(sml::state<flowsm_giving>)>(
            sml::state<BROKE>));
    }
