...
int main()
{
    ...
    // from port::send()
    zmsg_t* msg = zmsg_new();
    zmsg_addstrf(msg, "ZIO%d%s%s", 4, "TEXT",
                 "Live long int and be unsigned");
    ...
    std::string gots = got;
    free (got);
    cerr << "Size: " << gots.size() << " " << strlen(got)
         << ": " << gots << endl;
    ...
    cerr << "h2: "
         << std::hex << std::uppercase << " o=0X" << origin
         << " (" << std::dec << origin << " )"
         << std::hex << std::uppercase << " g=0X" << granule
         << " (" << std::dec << granule << " )"
         << endl;
