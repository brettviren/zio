...
int main(int argc, char* argv[])
{
    ...
    assert(uuids.size() == 1);
    if (verbose) cerr << "pub uuid is " << uuids[0] << endl;
    auto pubinfo = subpeer.peer_info(uuids[0]);
    if (verbose) cerr << "pub nick is " << pubinfo.nick << endl;
    assert(pubinfo.nick == "pub");
    assert(pubinfo.headers.size() == 1);
    auto feeds = pubinfo.branch("Feed");
    if (verbose) cerr << "got " << feeds.size() << ": " << feeds[""] << endl;
    assert(feeds.size() == 1);
