...
void Broker::purge_workers()
{
    ...
    for (auto wrk : m_waiting) {
        if (wrk->expiry <= now) { dead.push_back(wrk); }
    }
    for (auto wrk : dead) {
        zio::debug("zio::domo::Broker deleting expired worker: {}",
                   wrk->identity);
...
void Broker::service_internal(remote_identity_t rid, std::string service_name,
                              zio::multipart_t& mmsg)
{
    ...
    if (service_name == "mmi.service") {
        ...
        if (srv and srv->nworkers) { response.pushstr("200"); }
        else {
...
void Broker::service_dispatch(Service* srv)
{
    ...
    while (srv->waiting.size() and srv->requests.size()) {
        ...
        for (++next; next != srv->waiting.end(); ++next) {
            if ((*next)->expiry > (*wrk_it)->expiry) { wrk_it = next; }
        }
...
void Broker::worker_delete(Broker::Worker*& wrk, int disconnect)
{
    ...
    if (wrk->service) {
        for (std::list<Worker*>::iterator it = wrk->service->waiting.begin();
            ...
            if (*it == wrk) { it = wrk->service->waiting.erase(it); }
            else {
                ++it;
            }
        }
...
void Broker::worker_process(remote_identity_t sender, zio::multipart_t& mmsg)
{
    ...
    if (mdp::worker::ready == command) {
        ...
        //     zio::error("zio::domo::Broker protocol error (worker mmi) from:
        //     {}", sender); worker_delete(wrk, 1); return;
        // }
...
void zio::domo::broker_actor(zio::socket_t& link, std::string address,
                             int socktype)
{
    zio::context_t ctx;
    ...
    while (!interrupted()) {
        ...
        for (int iev = 0; iev < nevents; ++iev) {
            ...
            if (events[iev].socket == link) {
                ...
                zio::message_t msg;
                auto res =
                    events[0].socket.recv(msg, zio::recv_flags::dontwait);
                assert(res);
                std::stringstream ss;
                ss << "msg: " << msg.size();
        ...
        if (!nevents) { zio::debug("broker actor timeout"); }
