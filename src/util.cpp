                                       ...
                                       recv_flags flags)
{
    int stype = sock.getsockopt<int>(ZMQ_TYPE);
    if (ZMQ_SERVER == stype) {
        return recv_server(sock, mmsg, remid);
    }
    if(ZMQ_ROUTER == stype) {
        return recv_router(sock, mmsg, remid);
    }
                                       ...
                                       send_flags flags)
{
    int stype = sock.getsockopt<int>(ZMQ_TYPE);
    if (ZMQ_SERVER == stype) {
        return send_server(sock, mmsg, remid);
    }
    if(ZMQ_ROUTER == stype) {
        return send_router(sock, mmsg, remid);
    }
                                       ...
                                       recv_flags flags)
{
    ...
    if (ZMQ_CLIENT == stype) {
        return recv_client(socket, mmsg);
    }
    if(ZMQ_DEALER == stype) {
        return recv_dealer(socket, mmsg);
    }
                                       ...
                                       send_flags flags)
{
    int stype = socket.getsockopt<int>(ZMQ_TYPE);
    if (ZMQ_CLIENT == stype) {
        return send_client(socket, mmsg);
    }
    if(ZMQ_DEALER == stype) {
        return send_dealer(socket, mmsg);
    }
