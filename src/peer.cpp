...
zio::headerset_t zio::peer_info_t::branch(const std::string& prefix)
{
    ...
    for (const auto& hh : headers) {
        if (hh.first.substr(0, len) != prefix) { continue; }
    ...
    , m_zyre(nullptr)
{
    ...
    if (m_verbose) { zyre_set_verbose(m_zyre); }
    for (const auto& header : headers) {
        if (verbose)
            zio::debug("[peer {}]: header {} = {}", m_nick, header.first,
                       header.second);
        zyre_set_header(m_zyre, header.first.c_str(), "%s",
                        header.second.c_str());
    }
    // fixme: starting this here negates the user doing various useful
    // things with the zyre node.
    int rc = zyre_start(m_zyre);
    if (rc < 0) { throw std::runtime_error("failed to start zyre"); }
}
zio::Peer::~Peer()
{
    ...
    while (true) {
        ...
        if (streq(event_type, "ENTER")) {
            ...
            while (cursor) {
                ...
                pi.headers[key] = val;
                zio::debug("[peer {}]: poll add {} header {}={}", m_nick,
                           pi.nick, key, val);
                cursor = zlist_next(keys);
            }
            m_known_peers[uuid] = pi;
            zlist_destroy(&keys);
            zio::debug("[peer {}]: poll add \"{}\" ({}), know {}", m_nick,
                       pi.nick, uuid, m_known_peers.size());
            got_one = true;
        }
        else if (streq(event_type, "EXIT")) {
            uuid_t uuid = zyre_event_peer_uuid(event);
            peerset_t::iterator maybe = m_known_peers.find(uuid);
            if (maybe != m_known_peers.end()) { m_known_peers.erase(maybe); }
            zio::debug("[peer {}]: poll remove {}, know {}", m_nick, uuid,
                       m_known_peers.size());
            got_one = true;
        }
        ...
        if (!got_one) {
            if (timeout < 0) {
                zio::debug("[peer {}]: poll again with infinite wait", m_nick);
                continue;
            }
            auto timeleft = zclock_usecs() / 1000 - start - timeout;
            if (timeleft > 0) {
                zio::debug("[peer {}]: poll again with {} msec left", m_nick,
                           timeleft);
                continue;
            }
        }
...
std::vector<zio::uuid_t> zio::Peer::nickmatch(const nickname_t& nickname)
{
    ...
    for (const auto& pp : m_known_peers) {
        zio::debug("[peer {}]: check nick '{}' against '{}'", m_nick, nickname,
                   pp.second.nick);
        if (pp.second.nick == nickname) { ret.push_back(pp.first); }
    }
    zio::debug("[peer {}]: nickmatch found {} instances of {}", m_nick,
               ret.size(), nickname);
    return ret;
}
                                            ...
                                            timeout_t timeout)
{
    ...
    while (maybe.empty()) {
        zio::debug("[peer {}]: waitfor peer \"{}\" to come online", m_nick,
                   nickname);
        poll(timeout);
        maybe = nickmatch(nickname);
        zio::debug("[peer {}]: waitfor see \"{}\" after {} ms, have {} match",
                   m_nick, nickname, zclock_usecs() / 1000 - start,
                   maybe.size());

        if (maybe.size()) { break; }
        if (timeout < 0) { continue; }
        if (zclock_usecs() / 1000 - start >= timeout) { break; }
    }
    return maybe;
}
...
zio::peer_info_t zio::Peer::peer_info(const uuid_t& uuid)
{
    ...
    if (maybe == m_known_peers.end()) {
        zio::debug("[peer {}]: failed to find peer {} out of {}", m_nick, uuid,
                   m_known_peers.size());
        return peer_info_t{};
    }
    return maybe->second;
